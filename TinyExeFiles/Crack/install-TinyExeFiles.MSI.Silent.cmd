...
rem
rem echo Uninstall Previous Copy...
rem
%WMICEXE% product where name=%SFXNAME% call uninstall >NUL 2>NUL
rem
rem echo Install %SFXARCH%...
rem
%MSIEXECEXE% /i %SFXARCH% /qn /quiet /norestart /L*V %TPDL%\%SFXARCH%.log
rem %MSIEXECEXE% /i %SFXARCH% /qf /L*V %TPDL%\%SFXARCH%.log
goto Finish
:Error
rem echo "File %FOLDER%\%SFXARCH% not found" && exit /b 1
rem pause
:Finish
